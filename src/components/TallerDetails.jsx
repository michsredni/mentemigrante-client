...
function TallerDetails(props) {
  const { creador, descripcion, duracion, imagen, nombre, usuarios, _id } =
    props.eachTaller;
    ...
    return (
      <Spinner animation="border" role="status">
        <span className="visually-hidden">
          Buscando información del taller...
        </span>
      </Spinner>
    );
  }
  ...
  return (
    <div>
      <Card className="taller-details-card">
        <div className="taller-card-image-div">
          <Card.Img
            variant="top"
            src={imagen}
            alt="imagen-taller"
            style={{ width: "20vw", maxHeight: "50vw" }}
          />
        </div>
        <div className="taller-card-info-div">
          <Card.Body>
              ...
              <ListGroup.Item>
                <p>
                  ...
                  {usuarios.map((eachUsuario) => {
                    return (
                      <li key={eachUsuario._id}>
                        {eachUsuario.nombreCompleto}{" "}
                      </li>
                    );
                  })}
                </p>
            ...
            <div>
              ...
              {isUsuario && estaRegistradoTaller && (
                <Link>
              ...
              {creador._id == idUsuarioLoggeado ? (
                <>
                  {" "}
                  <Link to={`/talleres/${_id}/editar`}>
                    <button>Editar</button>
                  </Link>{" "}
                  <Link>
                    <button onClick={deleteTaller}>Borrar</button>
                  </Link>{" "}
                </>
              ) : null}
            </div>
